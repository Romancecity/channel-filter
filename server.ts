...
function checkStreamUrl(streamUrl: string, timeoutMs: number = 5000): Promise<{ status: 'working' | 'dead'; httpCode: number; responseTimeMs: number; errorReason?: string }> {
    ...
    try {
      ...
      const req = client.request(requestOptions, (res) => {
        ...
        res.on("end", () => {
          ...
          // Strictly check for direct stream status:
          // Must be HTTP 200/206
          if (statusCode >= 200 && statusCode < 400) {
            // Check if response is HTML error page instead of a real video stream
